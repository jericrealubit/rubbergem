...
The split rows that already exist are corrected on two fronts. `resolveShiftRows` (`lib/shift-log.ts`, used by all three History views) folds a spillover row into the previous day's night shift on read, so nothing renders twice before the database is touched; `night_shift_midnight_fix.sql` then repairs the rows themselves. `isNightMidnightSpillover` is the shared test — a night row is spillover when _every_ entry ends before the rollover hour, which a shift that genuinely started that date can never satisfy because it has evening entries. A spillover row with no shift to merge into is reported, never moved: a re-dated row still looks like spillover, so re-dating is a correction that cannot be applied twice.
