...
**The reject reasons all live in one radio group.** `selectedTableSquares[tableId]` holds a single value per table per cycle, and the whole vocabulary passes through it: the 5 grid positions, `bubble`, and `b-grade` (`B_GRADE_POSITION` in `lib/shift-log.ts`). The form renders the last two as the stacked pair beside each table's position grid — the vertical strip that used to be one "Bubble" button, now split into a **B** half (B-grade) over a bubble-icon half. Because they are values of the same `RadioGroup`, "max 1 reject per table per cycle" holds by construction rather than by arithmetic: picking B-grade clears any position and vice versa, so the write path's `isReject = !!shortMoldPos` needed no change to take a third reason on.

**B-grade is a _breakdown of_ the rejects, not a bucket beside them.** A B-grade mat is `good: 0, reject: 1` like any other, so `good + reject` is still the cycle count and every total taken off it — `total_mats_produced` / `faulty_mats_produced`, `pressMatTotals`, the wallboard's reject rate, History's month figures — keeps counting each mat exactly once and needed no change. What's new is the count beside them: `tableYieldsFromCycles` tallies `b_grade` per table by testing the position (the position is the single record of it; a separate boolean would be free to drift out of sync with `reject`), and the displays that break the rejects down read it — the live table's yellow `B:` chip in "Table Line Output Yields" (`getTableStats`) and History's per-table `B-GRADE` column. The `/tv` Defect Location Heatmap tallies it too, off the position rather than the yields, as a `B-Grade` cell paired with `Bubble` on one row beneath the position grid (neither is a place on the table). It is the one cell on `/tv` not drawn from the `--destructive` ramp: `countToAmberBucket` (`lib/heatmap-color.ts`) keeps `countToRedBucket`'s exact thresholds so magnitudes stay comparable, and only the hue says a downgrade is not scrap. That pair is one row and not two on purpose — a second row overflowed the card at 1280x720, where `/tv`'s `overflow-hidden` clips rather than scrolls. Rows archived before `b_grade` existed have no such key and read as 0. Anything adding `G + R + B` is double-counting; the `title` tooltips on those figures say so. `formatRejectPosition` (`lib/shift-log.ts`) is the one place a stored slug becomes display text — `b-grade` → "B-Grade", and the rest title-cased with it so the defect column doesn't mix casings.

It also means **`production_logs.total_mats_produced` is the _good_ count, not every mat pressed** — the write path sums per-table `good` into it and per-table `reject` into `faulty_mats_produced` beside it. All mats is the two added, which is the figure the live table shows as "cycles × 4 tables", the wallboard KPI row as "Mats Produced" and History as a shift's `mats:`. Anything taking a reject rate off an archived row must use `pressMatTotals` (`lib/shift-log.ts`) rather than dividing by `total_mats_produced`, which is rejects over good and reads high. The `/tv` trend heatmap and History's month header both did exactly that until they were fixed — the month header totalled the stored column while the day rows beneath it showed good + reject, so a month reported fewer mats than its own days and fewer than its own `G:` beside it. Both now derive `mats:` from the per-table yields, and the month figure is the sum of its day rows by construction. (Bales is _not_ the same shape: `bales_produced` and `faulty_bales_count` are two independent operator-entered numbers per cycle, faulty being a subset of produced, so `total_faulty_bales / total_bales_produced` is already the right rate there, and its month header sums real totals.)
...
**The shift date is not the calendar date.** The night shift runs into the small hours — rostered to finish around 00:30, sometimes later — so a cycle logged at 00:19 belongs to the shift that _started the previous evening_. `currentDate` in all three forms is therefore `currentShiftDate(shift)` (`lib/shift-log.ts`), which is today's Perth date for a day shift but _yesterday's_ for a night shift before `NIGHT_SHIFT_ROLLOVER_HOUR` (06:00, chosen because no night shift ever starts before it). It is recomputed on a 30s timer, not once on mount, so the rollover happens under a terminal that stays open. This matters in three places at once, and they are the same bug:

- **One row per shift.** `currentDate` is what `findShiftLogRow` looks the archive row up by and what gets written to `*_production_logs.date`. Stamping the raw calendar date meant that at midnight the shift stopped matching its own row and a second row opened on the next day holding only the after-midnight cycles — a one-cycle "night shift" on a day nobody worked, with those cycles missing from the real shift's totals.
- **The stale-clear dialog.** Same lookup: with no row found for the new date, a terminal submitting at 00:19 was offered the _running_ shift's `live_log` rows as "stale". That is how the split rows ended up holding a single cycle rather than a re-archive of the whole shift.
- **Cycle timestamps.** `shiftTimestamp(shiftDate, hhmm, shift)` places a Perth `HH:mm` relative to the shift's own date, so a night shift's small hours land on the _next_ calendar day. Pinning both ends to `currentDate` (what the forms used to do) gave a 23:35 → 00:19 cycle an end timestamp _before_ its start and a negative `run_duration_seconds`.

Ordering follows the same rule: on a night shift "00:19" sorts _after_ "23:35", not to the top. `mergeCycles` takes the shift group for exactly this, and `compareShiftCycles`/`shiftTimeRank` are the comparator the History table views use.

The split rows that already exist are corrected on two fronts. `resolveShiftRows` (`lib/shift-log.ts`) folds a spillover row into the previous day's night shift on read, so nothing renders twice before the database is touched; `night_shift_midnight_fix.sql` then repairs the rows themselves. Every read path goes through it — all three History views and `useShiftArchive` in `components/tv/use-tv-data.ts`, which the wallboard's shift picker and trend heatmap share — and each supplies its own line's fold (`mergePressShiftRows` / `mergeBalesShiftRows` / `mergeBanburyShiftRows`), because the totals differ: Press recomputes per-table yields from the merged cycles where they carry `short_mold_json` and otherwise adds the stored figures, Bales recomputes from cycles as its write path does, and Banbury takes the later snapshot of its operator-entered scalars rather than adding two views of the same running totals. Since a fold merges into the host row and an orphan is never moved, every resolved row's own `date` is already its shift's date, so callers keying off `row.date` stay correct. Note `useShiftArchive` takes that fold as a function argument, the one non-primitive in a file whose arguments are otherwise primitives to keep effect dependency lists stable — it feeds the resolution `useMemo`, not the fetch effect, so callers must pass the module-level import rather than an inline arrow. `isNightMidnightSpillover` is the shared test — a night row is spillover when _every_ entry ends before the rollover hour, which a shift that genuinely started that date can never satisfy because it has evening entries. A spillover row with no shift to merge into is reported, never moved: a re-dated row still looks like spillover, so re-dating is a correction that cannot be applied twice.
